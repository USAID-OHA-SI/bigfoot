...
#UPDATE: Added additional MER indicators (SC_CURR and TX_New) and left in all periods
...
get_scfact_mer <- function(input) {
  ...
  df <- ICPIutilities::read_msd(input) %>% 
    filter(indicator %in% c("TX_CURR", "TX_NEW", "SC_ARVDISP", "SC_CURR"),
           indicator == "TX_CURR" & standardizeddisaggregate %in% c("Age/Sex/ARVDispense/HIVStatus",
                                                                    "Total Numerator") |
             indicator == "TX_NEW" & standardizeddisaggregate == "Total Numerator" |
             indicator %in% c("SC_ARVDISP", "SC_CURR"))
}
